...
class PriceDaoTest {
    ...
    private void verifyFindFirstByBrandIdAndProductIdAndStartDateLessThanEqualAndEndDateGreaterThanEqualOrderByStartDateDesc(LocalDateTime applicationDate, PriceEntity priceEntity) {
        ...
        List<PriceEntity> prices = priceDao.findFirstByBrandIdAndProductIdAndStartDateLessThanEqualAndEndDateGreaterThanEqualOrderByStartDateDesc(BRAND_ID_DEFAULT, PRODUCT_ID_DEFAULT, applicationDate, applicationDate);
        log.info(prices.toString());
        assertEquals(priceEntity, prices.get(0));
    }
}